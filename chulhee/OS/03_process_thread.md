...
- 힙
  ...
  - **그렇다면 파이썬은 포인터를 사용하지 않는데??**
    ...
    - 그럼 파이썬은 스택 영역만 사용하게 되는가, 그렇지 않습니다. 파이썬은 메모리를 관리해주는 특별한 기능이 있습니다. Python Memory Manager라는 기능인데요, 이 기능이 포인터를 움직여 힙 영역의 메모리 할당 범위와 내부 버퍼를 조정해줍니다. Python Memory Manager는 Python/C API를 통하여 스토리지를 동적으로 관리합니다. 
